...
Branchrouter.put("/updatependingparcelstatus/:id", async (req, res) => {
    try {
        ...
        if (updateData.parcelstatus == "Received") {
            ...
            const supdateData = await Parcel.updateOne({ referancenumber: updateData.referancenumber }, {mainparcelstatus: "Delivered",deliverydate: moment(updateData.updatedAt).format('L') , flag:1 }, {
                new: true
            })
        }
